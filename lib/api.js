...
export const getCopilotUsageForTeam = async (org, team, startDate, endDate) => {
  try {
    ...
    // Save raw data in the summary for reference (exact format from API)
    summary.rawData = metricsData;
    // Store the team name
    summary.team_slug = team;
    return summary;
  } catch (error) {
...
export const processMetricsData = (metricsData) => {
  ...
  if (summary.processedDays > 1) {
    ...
    // Calculate the normalization factor for consistent comparison across time periods
    // The factor is the actual number of days in the data divided by the standard period (28 days)
    // This ensures metrics show comparable values regardless of time period selected
    const normalizationFactor = summary.processedDays / 28;
    
    console.log(`Normalizing data with factor: ${normalizationFactor} (processed days: ${summary.processedDays})`);
    
    summary.languages.forEach(lang => {
      if (!lang) return;
      
      // Normalize language data to compensate for time period differences
      // We preserve the original values for calculations that need raw numbers
      lang.raw_total_suggestions = lang.total_suggestions;
      lang.raw_total_acceptances = lang.total_acceptances;
      lang.raw_total_lines_suggested = lang.total_lines_suggested;
      lang.raw_total_lines_accepted = lang.total_lines_accepted;
      ...
      // Don't normalize the acceptance rate as that should be consistent across time periods
      // Instead, ensure it doesn't exceed 100%
    ...
    summary.editors.forEach(editor => {
      ...
      // Normalize editor data to compensate for time period differences
      // We preserve the original values for calculations that need raw numbers
      editor.raw_total_suggestions = editor.total_suggestions;
      editor.raw_total_acceptances = editor.total_acceptances;
      editor.raw_total_lines_suggested = editor.total_lines_suggested;
      editor.raw_total_lines_accepted = editor.total_lines_accepted;
