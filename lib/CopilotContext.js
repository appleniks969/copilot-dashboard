...
export function CopilotProvider({ children }) {
  ...
  const updateReportDateRange = async (reportId, newDateRange) => {
    // Skip if already loading or if this date range is already loaded for this report
    const loadedKey = `${reportId}-${newDateRange}-${organization}-${team}`;
    const isAlreadyLoaded = loadedReports[loadedKey];
    
    // Don't trigger unnecessary state updates if the date range is already set correctly
    ...
    requestDebounceTimers.current[reportId] = setTimeout(async () => {
      // If data for this date range is already loaded, skip the API call
      if (isAlreadyLoaded && reportData[reportId]) {
        console.log(`Data already loaded for ${reportId} with range ${newDateRange}, skipping fetch`);
        return;
      }
